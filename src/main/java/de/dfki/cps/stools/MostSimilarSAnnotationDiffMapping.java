 ...
 */
public class MostSimilarSAnnotationDiffMapping extends MostSimilarSubsetMapping<SAnnotation<?>> {
    private List<SAnnotation<?>> X;
    private List<SAnnotation<?>> Y;
    private STools manager;


    public MostSimilarSAnnotationDiffMapping () {}

    public MostSimilarSAnnotationDiffMapping(STools m, List<SAnnotation<?>> l, List<SAnnotation<?>> r) {
        super();
        X = l;
        Y = r;
        manager = m;
    ...
    public void initFrom(MostSimilarSubsetMapping<SAnnotation<?>> other) {
        if (other instanceof MostSimilarSAnnotationDiffMapping) {
            manager = ((MostSimilarSAnnotationDiffMapping) other).manager;
        }
    }
    ...
    protected double similarity(SAnnotation<?> x1, SAnnotation<?> y1) {
        if (x1.getNameSpace().equals(y1.getNameSpace())
            ...
            if (x1.getValue().equals(y1.getValue())) return 1.0;
            else return 0.9;
        }
        else return 0.0;
    }
